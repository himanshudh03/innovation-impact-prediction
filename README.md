...
MSc coursework project (Advanced Analytics & Machine Learning, Queen's University Belfast) - a full applied ML workflow on a 35,308-record biomedical research dataset, testing which factors actually drive citation impact: novelty, knowledge-network position, or author collaboration.

## Problem

Citation counts are extremely right-skewed (skewness 18.7 raw, 0.27 after log transform) and driven by overlapping, correlated signals: novelty scores, topic breadth, and collaboration-network position all move together. The goal was to find which of these genuinely predicts impact once multicollinearity is accounted for, using a full model-comparison workflow rather than a single method.
...
- - Non-linear modelling: pruned decision tree and random forest (rpart, ranger) to capture thresholds and interactions LASSO can't see, with permutation importance and partial dependence plots.
  - - Classification: LDA, linear SVM and radial SVM (MASS, e1071) to classify high- vs low-citation records on a near-balanced 51/49 split, evaluated on accuracy, balanced accuracy, precision, recall and F1.
    - - Model complexity control: bias-variance analysis across random forest mtry values, comparing training/CV/test error to justify the final model choice.
      - - Non-linearity check: polynomial regression (linear/quadratic/cubic) on team size, showing an inverted-U relationship between collaboration size and citation impact.
        ...
        - Collaboration-network variables (Degree_Avg, Core_Number_Avg, AuthorCount, PageRank_Avg) consistently outranked novelty scores in predictive importance - team visibility and network position matter more than idea novelty alone, with diminishing (and eventually negative) returns for very large teams.
